...
int main() {
    Stack *stack = stack_create(10);  // объявление стека
    ...
    while ((symbol = getchar()) != '=') {       // считывание символов пока не поступит знак '='
        ...
        else if (symbol == '+') {               // если символ является одним из операторов:
            result = stack_pop(stack) + stack_pop(stack);  // выполнить соответствующую операцию
            stack = stack_push(stack, result);
        }
        else if (symbol == '-') {
            element2 = stack_pop(stack);        // Именно в вычитании нужно выносить в отдельные переменные,
            element1 = stack_pop(stack);        // снятые с вершины 2 элемента из-за undefined behaviour,
            result = element1 - element2;       // которое возникает 
            stack = stack_push(stack, result);  // при result = stack_pop(stack) - stack_pop(stack)
        }
        else if (symbol == '*') {
            result = stack_pop(stack) * stack_pop(stack);
            stack = stack_push(stack, result);
        }
    }
